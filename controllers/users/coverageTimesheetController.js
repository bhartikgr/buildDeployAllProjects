...
exports.getCoverageTimeSheet = (req, res) => {
  ...
  db.query(
    ...
    function (err, result, fields) {
      ...
      if (result != "") {
        var arr = createCoverageTimeSheetWeekRanges(result);
      } else {
        var arr = [];
      }
      res.json({ arr });
...
function createCoverageTimeSheetWeekRanges(dates) {
  const groups = dates.reduce((groups, dateObj) => {
    ...
    // Key: "2025-W05"
    const weekKey = `${date.getUTCFullYear()}-W${String(weekNumber).padStart(
      2,
      "0"
    )}`;
    ...
    groups[weekKey].push(dateObj);

    return groups;
  }, {});

  // ---- BUILD OUTPUT ----
  const output = Object.entries(groups).map(([weekKey, dates]) => {
