...
exports.getCoverageTimeSheet = (req, res) => {
  ...
  db.query(
    "SELECT attendance.* from rosters join attendance on  attendance.roster_id = rosters.id where rosters.user_id=? And attendance.hours_status =?  And attendance.roster = ? And attendance.client_id = ? And attendance.location_id = ? And rosters.id = ?",
    [
      data.user_id,
      "User",
      data.rostertype,
