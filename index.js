...
const corsOptions = {
  origin: ["https://jlmining.online", "http://localhost:3002"], // List of allowed origins
...
app.post("/admin/attendancesave", function (req, res) {
  ...
  let errorDates = []; // Dates already processed

  missingDates.forEach((datt) => {
    const dd = getdays(datt);

    // Step 1: Get all attendance records for this date
    db.query(
      "SELECT roster, activity_log FROM attendance WHERE user_id = ? AND date = ?",
      [data.user_id, dd],
      function (err, results) {
        ...
        // Case 1: Another roster already processed → block
        const otherRosterHasAttendance = results.some(
          (r) => r.roster !== currentRoster && r.activity_log === "Process"
        );

        if (otherRosterHasAttendance) {
          const existingRoster = results.find(
            (r) => r.roster !== currentRoster && r.activity_log === "Process"
          ).roster;
          ...
          errorDates.push({
            date: datt,
            existingRoster,
            message: `Date ${datt} already processed in roster ${existingRoster}`,
          });
          ...
          if (completedQueries === totalQueries && !responseSent) {
            res.status(200).json({
              ...
              errorDates,
              message:
                "Some attendance records were already processed in another roster and could not be updated",
            });
            responseSent = true;
          }
        ...
        // Case 2: Same roster already processed → block
        // const sameRosterProcessed = results.some(
        //   (r) => r.roster === currentRoster && r.activity_log === "Process"
        // );

        // if (sameRosterProcessed) {
        //   errorDates.push({
        //     date: datt,
        //     existingRoster: currentRoster,
        //     message: `Date ${datt} already processed in current roster`,
        //   });
        ...
        //     res.status(200).json({
        //       status: "2",
        //       error: "Some dates already processed in current roster",
        //       errorDates,
        //       message:
        //         "Some attendance records were already processed in current roster and could not be updated",
        //     });
        //     responseSent = true;
        //   }
        //   return; // 🚨 stop further update
        ...
        // Case 3: Not processed anywhere → ✅ update attendance
        let updateQuery = "";
        let updateParams = [];

        if (data.coverageStatus) {
          updateQuery =
            "UPDATE attendance SET client_id=?, location_id=?, shift=?, activity_log=? WHERE user_id = ? AND date=? AND roster = ? AND shift_type = 'Add'";
          updateParams = [
            data.clientId,
            data.location,
            data.shift,
          ...
          updateQuery =
            "UPDATE attendance SET client_id=?, location_id=?, shift=?, activity_log=? WHERE user_id = ? AND date=? AND roster = ?";
          updateParams = [
            data.clientId,
            data.location,
            data.shift,
        ...
        db.query(updateQuery, updateParams, function (err) {
          if (err) {
            if (!responseSent) {
          ...
          if (completedQueries === totalQueries && !responseSent) {
            if (errorDates.length > 0) {
              res.status(200).json({
                status: "2",
                error: "Some dates already processed",
                errorDates,
                message:
                  "Some attendance records were already processed and could not be updated",
              });
            } else {
...
app.post("/admin/getallCalendardetailForadminsearch", function (req, res) {
  ...
  var search = data.search;
  //console.log(typeof startDatee, "fffff", typeof endDatee);
  //console.log(data);
  db.query(
    `SELECT
  ...
  ORDER BY
    ...
    function (err, results, fields) {
      ...
      //console.log(results);
      if (check.length > 0) {
        // If `check` contains at least one value, then the date matched
        // console.log("s");
        const maindata = results;
        res.json({ maindata });
      } else {
        // If no matches were found, concatenate `currentarray` with results
        //console.log("sa");
        const maindata = results.concat(currentarray);
        res.json({ maindata });
      }
...
app.post("/admin/multiplerosteradd", function (req, res) {
  ...
  const currentRoster = data.rostertype;
  //console.log(d);
  db.query(
    `SELECT * 
     FROM attendance 
